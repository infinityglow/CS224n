...
def pad_sents_char(sents, char_pad_token):
    ...
    for sentence in sents:
        padded_sen = sentence + [[]] * (max_length - len(sentence))
        sents_.append(padded_sen)
    sents_padded = []
    for sentence in sents_:
        sentence_pad = []
        for word in sentence:
            word_padded = word + [char_pad_token] * (max_word_length - len(word))
            sentence_pad.append(word_padded)
        sents_padded.append(sentence_pad)
    ### END YOUR CODE
...
def pad_sents(sents, pad_token):
    ...
    max_length = max([len(sentence) for sentence in sents])
    for sentence in sents:
        padded_sen = sentence + [pad_token] * (max_length - len(sentence))
