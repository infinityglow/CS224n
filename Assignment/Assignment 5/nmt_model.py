...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2, no_char_decoder=False):
        ...
        self.model_embeddings_source = ModelEmbeddings(embed_size, vocab.src, hidden_size)
        self.model_embeddings_target = ModelEmbeddings(embed_size, vocab.tgt, hidden_size)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for y_t in torch.split(Y, 1, dim=0):
            y_bar_t = torch.cat([torch.squeeze(y_t, dim=0), o_prev], dim=1)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.squeeze(torch.bmm(torch.unsqueeze(dec_hidden, dim=1), enc_hiddens_proj.permute([0, 2, 1])), 1)
